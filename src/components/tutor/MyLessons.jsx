            ...
            <div key={item.id} className="m-1 mb-3 p-4 bg-white" style={{borderRadius: '3px', marginBottom: '5px'}}>
              <Row>
                ...
                <Col md={12} lg={6} className="my-lessons-area">
                  ...
                  <div style={{display: 'flex', justifyContent: 'space-between'}}><b>{t('email')}:</b> {item.email}
                  </div>
                  ...
                  {item.last &&
                    <>
                      <button style={{fontSize: '14px', borderRadius: '3px', height: '53px'}}
                              ...
                              disabled={item.last.first_confirmed == 'tutor' ? true : false}
                              onClick={() => confirmLesson(item.last ? item.last.id : 0, item.last.start_time)}>{item.last.first_confirmed == 'tutor' ? t('waiting_student_confirmation') : t('confirm_last_lesson')} </button>
                ...
                <div className="d-flex mt-4 my-lesson-activity">
                  <div style={{cursor: 'pointer', marginRight: '25px', color: '#666666'}}
                       onClick={() => quit(item.id, item.count_done)}><Trash color="#666666"
                                                                             size={20}/> {t('quit_tutoring')}</div>
                  <div style={{cursor: 'pointer', marginRight: '25px'}}><Link style={{color: '#666666'}}
                                                                              to={'/tutor/reschedule/' + item.id}><CalendarDate
                    color="#666666" size={20}/> {t('change_lesson_dates')}</Link></div>
